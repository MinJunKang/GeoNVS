...
class UNetSpatioTemporalConditionModelPoseCond(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin):
    r"""
    ...
    def forward(
        ...
        if use_gs_adapter:
            hs_augmentation = [down_block_res_samples[iidx] for iidx in gs_modules["input_layer_indices"]]
            
            # multi scale feature fusion
            fused_feature = gs_modules["multi_fusion"](hs_augmentation)[0]
            
            # feature aware gs attention
            updated_h, feature_rec_loss = gs_modules["gsattn"](
            ...
            novel_h = gs_modules["input_fusion"](novel_h, updated_h)
            ...
            # simple upsampling
            hs_gs_augmentation = gs_modules["input_decoder"](fused_feature, hs_augmentation)
            for iidx in gs_modules["input_layer_indices"]:
                down_block_res_samples[iidx] = hs_gs_augmentation.popleft()
        ...
        if not training_mode:
            return sample, intermediate_hs

        return sample, feature_loss