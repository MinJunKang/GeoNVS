...
class EncoderDepthSplat(Encoder[EncoderDepthSplatCfg]):
    ...
    def forward(
        ...
        match_prob = torch.max(match_prob, dim=1, keepdim=True)[0]  # [BV, 1, H, W]
        ...
        if self.cfg.supervise_intermediate_depth and len(depth_preds) > 1:
            ...
            gaussians = self.gaussian_adapter.forward(
                ...
                (h, w),
                input_images=sh_input_images.float().repeat(
            ...
            gaussians = self.gaussian_adapter.forward(
                ...
                (h, w),
                input_images=sh_input_images.float() if self.cfg.init_sh_input_img else None,
            )
        ...
        if visualization_dump is not None:
            ...
            visualization_dump["scales"] = rearrange(
                gaussians.scales.float(), "b v r srf spp xyz -> b (v r srf spp) xyz"
            )
            visualization_dump["rotations"] = rearrange(
                gaussians.rotations.float(), "b v r srf spp xyzw -> b (v r srf spp) xyzw"
            )
