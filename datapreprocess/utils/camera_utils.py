...
import cv2
import torch
from scene_custom.cameras import Camera
import numpy as np
...
def loadCam(resolution, id, cam_info, resolution_scale, data_device):
    ...
    resized_depth = cv2.resize(cam_info.depth, resolution, interpolation=cv2.INTER_NEAREST)
    resized_depth = torch.as_tensor(resized_depth, dtype=torch.float)[None]
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  image=gt_image, depth=resized_depth, gt_alpha_mask=loaded_mask, uid=id, data_device=data_device)


def cameraList_from_camInfos(cam_infos, resolution_scale, resolution, data_device):
    camera_list = []

    for id, c in enumerate(cam_infos):
        camera_list.append(loadCam(resolution, id, c, resolution_scale, data_device))
...
def visualizer(camera_poses, colors, save_path="camera_poses.png"):
    fig = plt.figure()
