...
from utils.graphics_utils import rotate_vector_to_vector
from scene_custom.per_point_adam import PerPointAdam
...
class GaussianModel:
    ...
    def setup_functions(self):
        ...
        def build_covariance_from_scaling_rotation2(scaling, scaling_modifier, rotation):
            L = build_scaling_rotation(scaling_modifier * scaling, rotation)
            actual_covariance = L @ L.transpose(1, 2)
        ...
        self.covariance_activation2 = build_covariance_from_scaling_rotation2
    ...
    def get_covariance_mat(self, scaling_modifier = 1):
        return self.covariance_activation2(self.get_scaling, scaling_modifier, self._rotation)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_point_cloud = pcd.points.to(self.device)
        fused_color = RGB2SH(pcd.colors.float().to(self.device))
        ...
        rots_default = torch.zeros((fused_point_cloud.shape[0], 4), device=self.device)
        rots_default[:, 0] = 1
        if pcd.normals is not None:
            normal_seed = pcd.normals.to(self.device)
            normal_seed = torch.nn.functional.normalize(normal_seed, dim=-1, eps=1e-8)
            mat = rotate_vector_to_vector(torch.tensor([0, 0, 1], dtype=torch.float, device=normal_seed.device).repeat(normal_seed.shape[0], 1), normal_seed)
            rots = matrix_to_quaternion(mat)
            nan_mask = torch.isnan(rots).any(dim=1)
            rots[nan_mask] = rots_default[nan_mask]
        else:
            rots = rots_default
    ...
    # per-point optimizer
    def training_setup_pp(self, training_args: dict, confidence_lr=None):
        self.percent_dense = training_args["percent_dense"]
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device=self.device)
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device=self.device)
    ...
    def _prune_optimizer(self, mask):
        ...
        for group in self.optimizer.param_groups:
            ...
            if stored_state is not None:
                ...
                group["params"][0] = nn.Parameter(group["params"][0][mask].requires_grad_(True))
                optimizable_tensors[group["name"]] = group["params"][0]
            per_point_lr = group.get('per_point_lr', None)
