...
def get_diffusion_model(dataset_path, modelname, weight_dtype, device,
    ...
    elif "lora_cameractrl" in modelname:
        model = CameraCtrlLoRAModel(
            dataset_path, modelname, weight_dtype, device,
            weight_path=camctrl_weight_path,
        )
...
class CameraCtrlLoRAModel(nn.Module):
    def __init__(self, dataset_path, model_name, weight_dtype, device, weight_path=None):
        super().__init__()
        self.dataset_path = dataset_path
        self.model_name = model_name
        self.weight_dtype = weight_dtype
        ...
        # LoRA weights (overridable via demo_diffusion.py --camctrl_weight_path)
        if weight_path is None:
            weight_path = "pretrained_weights/cameractrl_lora_145K/pytorch_lora_weights.safetensors"
        checkpoint_lora = weight_path
        self.gs_config = OmegaConf.load("configs/module_config/lora_only.yaml")
        
        use_gs_adapter = self.gs_config['use_gs_adapter']
        add_lora = self.gs_config['add_lora']
