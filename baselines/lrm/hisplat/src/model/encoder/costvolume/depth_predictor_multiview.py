...
class DepthPredictorMultiViewPyramid(nn.Module):
        ...
        for i in range(len(self.depth_predictor)):
            ...
            if i != 0:
                ...
                raw_gaussians_pre, raw_gaussians_now, mask_return = self.extract_raw_guassians(
                    raw_gaussians, image_size, i, result_dict, mask, gaussian_dict, densities, with_density=True
                )
    ...
    def extract_raw_guassians(
        self, raw_gaussians, original_size, stage_id, result_dict, mask, gaussian_dict, densities_now, with_density=False
    ):
        ...
        for j in range(stage_id):
            ...
            ds_factor = (r / r_pre) ** 0.5
            # h_pre, w_pre = int(r_pre**0.5), int(r_pre**0.5)
            h_pre, w_pre = int(h / ds_factor), int(w / ds_factor)
