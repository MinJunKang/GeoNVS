...
def feature_add_position_list(features_list, attn_splits, feature_channels):
    ...
    if attn_splits > 1:  # add position in splited window
        ...
        features_splits = [x + position.to(dtype=x.dtype) for x in features_splits]
        ...
        out_features_list = [x + position.to(dtype=x.dtype) for x in features_list]
...
class BackboneMultiview(torch.nn.Module):
    ...
    def forward(
        ...
        images = self.normalize_images(images).to(dtype=ori_dtype)
