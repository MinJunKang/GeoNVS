...
from baselines.diffusion.genfusion.lvdm.utils import instantiate_from_config
from baselines.diffusion.genfusion.lvdm.ema import LitEma
from baselines.diffusion.genfusion.lvdm.models.samplers.ddim import DDIMSampler
from baselines.diffusion.genfusion.lvdm.distributions import DiagonalGaussianDistribution
from baselines.diffusion.genfusion.lvdm.models.utils_diffusion import make_beta_schedule, rescale_zero_terminal_snr
from baselines.diffusion.genfusion.lvdm.basics import disabled_train
from baselines.diffusion.genfusion.lvdm.common import extract_into_tensor, noise_like, exists, default
...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        first_stage_depth_key="depth",
        cond_stage_key="caption",
        cond_stage_depth_key="artifact_depth",
        image_size=256,
        channels=3,
        log_every_t=100,
        clip_denoised=True,
        ...
        self.first_stage_depth_key = first_stage_depth_key
        self.cond_stage_key = cond_stage_key
        self.cond_stage_depth_key = cond_stage_depth_key
...
class LatentDiffusion(DDPM):
    ...
    def encode_first_stage(self, x):
        with torch.no_grad():
            ...
            latents = self.first_stage_model.encode(x).latent_dist.sample()
            latents = rearrange(latents, "(b f) c h w -> b c f h w", f=video_length)
            # latents = latents * self.first_stage_model.config.scaling_factor
    ...
    def decode_core(self, z, return_depth=False, **kwargs):
        with torch.no_grad():
    ...
    @torch.no_grad()
    def decode_first_stage(self, z, return_depth=False, **kwargs):
        return self.decode_core(z, return_depth, **kwargs)
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def get_batch_input(
        ...
        **kwargs,
        ...
        x = super().get_input(batch, self.first_stage_key)
        ## encode video frames x to z via a 2D encoder
        depth = super().get_input(batch, self.first_stage_depth_key)
        new_x = torch.cat([x, depth], dim=1)
        z = self.encode_first_stage(new_x)

        ## get caption condition
        cond_input = batch[self.cond_stage_key]

        cond = {}
        concat_rgb = super().get_input(batch, "artifact_rgb")
        ref_rgb = super().get_input(batch, "ref_rgb")
        concat_depth = super().get_input(batch, "artifact_depth")
        concat_cond = torch.cat([concat_rgb, concat_depth], dim=1)
        ...
        cross_attn_img = input_mask * ref_rgb  # [b l c h w]
        ## img: b c l h w
        cross_attn_img = rearrange(cross_attn_img, "b c l h w -> (b l) c h w")
        img_emb = self.embedder(cross_attn_img)  ## (b l) c
        img_emb = self.image_proj_model(img_emb)
        img_emb = rearrange(img_emb, "(b t) l c -> b (t l) c", b=x.shape[0])

        if self.model.conditioning_key == "hybrid":
            if self.interp_mode:
                ## starting frame + (L-2 empty frames) + ending frame
                img_cat_cond = self.encode_first_stage(concat_cond)
            else:
        ...
        cond["c_crossattn"] = [
            torch.cat([img_emb], dim=1)
        ]  ## concat in the seq_len dim

        out = [z, cond]
        if return_first_stage_outputs:
            xrec = self.decode_first_stage(z, return_depth=True)
            out.extend([xrec])
    ...
    def log_images(
        ...
        **kwargs,
        ...
        z, c, xrec, xc, fs, cond_x, ref_rgb = self.get_batch_input(
            batch,
            random_uncond=False,
            return_first_stage_outputs=True,
            return_original_cond=True,
        ...
        N = xrec.shape[0]
        log["image_condition"] = cond_x[:, :3]
        log["reconst"] = xrec[:, :3]
        log["reconst_depth"] = xrec[:, 3:4].repeat(1, 3, 1, 1, 1)
        xc_with_fs = []
        ...
        if sample:
            ...
            if unconditional_guidance_scale != 1.0:
                ...
                img = torch.zeros_like(ref_rgb)  ## b c h w
                img = rearrange(img, "b c l h w -> (b l) c h w")
                ## img: b c h w
                img_emb = self.embedder(img)  ## b l c
                uc_img = self.image_proj_model(img_emb)
                uc_img = rearrange(uc_img, "(b t) l c -> b (t l) c", b=xrec.shape[0])
                uc = torch.cat([uc_img], dim=1)
                ## hybrid case
                if isinstance(c, dict):
                    uc_hybrid = {"c_concat": [c_cat], "c_crossattn": [uc]}
                    uc = uc_hybrid
            ...
            with self.ema_scope("Plotting"):
                samples, z_denoise_row = self.sample_log(
                    ...
                    ddim_steps=ddim_steps,
                    eta=ddim_eta,
                    unconditional_guidance_scale=2.5,  # unconditional_guidance_scale,
                    unconditional_conditioning=uc,
                    x0=z,
                    **kwargs,
                )
            x_samples = self.decode_first_stage(samples, return_depth=True)
            log["samples"] = x_samples[:, :3]
            log["depth"] = x_samples[:, 3:4].repeat(1, 3, 1, 1, 1)
            log["depth_cond"] = cond_x[:, 3:4].repeat(1, 3, 1, 1, 1)
            print(
                "artifact_depth_tensor: ",
                cond_x[:, 3:4].shape,
                cond_x[:, 3:4].max(),
                cond_x[:, 3:4].min(),
            )
            print("x_samples: ", log["samples"].shape)
            print("depth:", x_samples[:, 3:].shape)
            print('log["depth"]: ', log["depth"].shape)
            if plot_denoise_rows:
                denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
                log["denoise_row"] = denoise_grid
