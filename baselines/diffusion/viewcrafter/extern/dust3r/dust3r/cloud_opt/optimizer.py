...
from baselines.diffusion.viewcrafter.extern.dust3r.dust3r.utils.geometry import xy_grid, geotrf
from baselines.diffusion.viewcrafter.extern.dust3r.dust3r.utils.device import to_cpu, to_numpy
...
class PointCloudOptimizer(BasePCOptimizer):
    ...
    def get_pts3d_from_depthmaps(self, depth, raw=False):
        # Get depths and  projection params if not provided
        focals = self.get_focals()
        pp = self.get_principal_points()
        im_poses = self.get_im_poses()
    ...
    def get_pts3d_from_depthmaps2(self, depth, focals, pp, im_poses, raw=False):
        # get pointmaps in camera frame
        rel_ptmaps = _fast_depthmap_to_pts3d(depth, self._grid, focals, pp=pp)
        # project to world frame
        res = geotrf(im_poses, rel_ptmaps)
