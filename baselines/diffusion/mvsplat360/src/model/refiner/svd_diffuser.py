...
class SVDDiffusion(DiffusionEngine):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs["sampler_config"].params.verbose = cfg.verbose
    ...
    def get_input(self, batch, **kwargs):
        ...
        batch["target_views"]: ground truth target views; for training only
        """

        # load the data
        device = kwargs.get("device", batch["context_views"].device)
        ...
        cond["cond_aug"] = (
            torch.tensor([self.cond_aug], dtype=context_views.dtype).repeat(b * v).to(device)
        )
        ...
        if self.motion_bucket_id is None:
            ...
            cond["motion_bucket_id"] = (
                torch.tensor([self.motion_bucket_id], dtype=context_views.dtype)
                .repeat(b * v)
                .to(device)
            )
        ...
        if self.fps_id is None:
            ...
            cond["fps_id"] = torch.tensor([self.fps_id], dtype=context_views.dtype).repeat(b * v).to(device)

        cond["num_video_frames"] = v
        cond["image_only_indicator"] = torch.zeros(b, v, dtype=context_views.dtype).to(device)
    ...
    def test_step(self, batch, batch_idx=None):
        ...
        weight_dtype = batch['context_views'].dtype
        device = batch['context_views'].device
        randn = batch.pop("randn", None)
        cond = self.get_input(batch, device=device)
        ...
        c['vector'] = c['vector'].to(dtype=weight_dtype, device=device)
        uc['vector'] = uc['vector'].to(dtype=weight_dtype, device=device)
        ...
        if randn is None:
            ...
            randn = torch.randn(
                (b * v, 4, int(h * self.cfg.gs_feat_scale_factor), 
                 int(w * self.cfg.gs_feat_scale_factor)), device=device
            )
