...
def main(
    ...
    seed=1234,
    dm_model_name='mvsplat360',
    normalize_camera=False,
    lrm_voxel_size=-1.0,  # negative value means not applied
    dir_tag='',
    # GS-Adapter config for --dm_model_name geonvs_cameractrl_* (default:
    # configs/module_config/gsadapter_camctrl_gattn.yaml)
    camctrl_gs_config=None,
    ...
    **overwrite_options,
    ...
    if lrm_model is not None:
        lrm_model.eval()
    MODEL = ModelWrapper(data_path, dm_model_name, torch.float32, device, lrm_model=lrm_model,
                         camctrl_gs_config=camctrl_gs_config,
                         camctrl_weight_path=camctrl_weight_path)
