...
from geonvs.seva.data_io import get_parser
from geonvs.seva.eval import (
    IS_TORCH_NIGHTLY,
...
from geonvs.seva.geometry import (
...
from geonvs.seva.modules.autoencoder import AutoEncoder
...
def parse_task(
    ...
    if task == "img2trajvid_s-prob":
        ...
        parser = get_parser(
            ...
            data_dir=scene,
            normalize=False,
            file_name="transforms.json"  # use transforms2.json when using vipe poses
        )
...
def main(
    ...
    gs_adapter_config=None,
    gs_adapter_weight_path=None,
    seed=1234,
    lrm_model_name='depthsplat',
    continuous_rendering=False,
    inpainting_mode=False,  # noisy render, inpainting mode
    inpainting_strength=0.2,  # only used in inpainting mode
    normalize_camera=False,
    lrm_downsample_factor=1,
    lrm_voxel_size=-1.0,  # negative value means not applied
    dir_tag='',
    enable_xformers=False,  # memory-efficient attention; faster but not bit-identical to the naive path
    **overwrite_options,
):
    continuous_rendering = bool(continuous_rendering)
    inpainting_mode = bool(inpainting_mode)
    num_inputs_opt = overwrite_options['num_inputs']
    data_path_tag = osp.basename(osp.normpath(data_path))
    num_frames = ([int(t) for t in T.split(",")] if isinstance(T, str) else T) or 21
    
    AE = AutoEncoder(chunk_size=1, num_frames=num_frames, use_temporal_decoder=True).to(device)
    ...
    if gs_adapter_config is None:
        ...
        with torch.device("meta"):
            MODEL = SGMWrapper(
                ...
                lrm_model=lrm_model,
                attn_mode="xformers" if bool(enable_xformers) else "naive",
            )
        
        # load pretrained weights
        state_dict_base = load_state_dict(
            model_version=version,
            pretrained_model_name_or_path=pretrained_model_name_or_path,
            weight_name=weight_name,
            device="cpu"
        )
        missing, unexpected = MODEL.module.load_state_dict(state_dict_base, strict=False, assign=True)
    else:
        # read gs adapter config
        use_gs_adapter, add_lora = False, False
        adapter_config = {}
        if gs_adapter_config:
            with open(gs_adapter_config, 'r') as f:
                adapter_config = yaml.safe_load(f)
            use_gs_adapter = adapter_config['use_gs_adapter']
            add_lora = adapter_config.get('add_lora', False)
        ...
        MODEL = SGMWrapper(
            ...
            max_resolution=(H or 576, W or 576),
            attn_mode="xformers" if bool(enable_xformers) else "naive",
        )
        ...
        # load lora weights
        if add_lora:
            lora_weight_path = osp.join(gs_adapter_weight_path, 'pytorch_lora_weights.safetensors')
            MODEL.module.load_lora_adapter(lora_weight_path, adapter_name="gs_lora", prefix=None)
        
        # load gs adapter weights
        if use_gs_adapter:
            gs_weight_path = osp.join(gs_adapter_weight_path, 'gs_adapter_weights.pth')
            gs_state_dict = torch.load(gs_weight_path, map_location="cpu", weights_only=True)
            ...
            for name in gs_state_dict.keys():
                ...
                getattr(MODEL.module, name).load_state_dict(sub_sd, strict=True)
            MODEL.module.gsattn.float()
    ...
    if data_items is not None:
        ...
        if osp.isfile(osp.join(data_path, "selected_scenes.txt")):
            with open(osp.join(data_path, "selected_scenes.txt"), 'r') as f:
                scenes = f.read().splitlines()
            print(f"Loaded {len(scenes)} selected scenes from selected_scenes.txt")
            scenes = [osp.join(data_path, s) for s in scenes if osp.isdir(osp.join(data_path, s))]
        else:
            scenes = [
    ...
    for scene in tqdm(scenes):
        ...
        if options.get("skip_saved", False) and os.path.exists(
            os.path.join(save_path_scene, "transforms2.json")
        ):
            print(f"Skipping {scene} as it is already sampled.")
            continue
        if os.path.exists(os.path.join(save_path_scene, "performance_table.xlsx")):
            print(f"Skipping {scene} as it is already sampled.")
            continue
        (
            all_imgs_path,
        ...
        video_path_generator = run_one_scene(
            task=task,
            scene_path=scene,
            version_dict=VERSION_DICT,  # H, W maybe updated in run_one_scene
            model=MODEL,
            ae=AE,
    ...
    # log final metrics
    metric_file_paths = glob.glob(os.path.join(WORK_DIR, task, "*", "performance_table.xlsx"))
    if len(metric_file_paths) > 0:
        mean_rows = []
        for path in metric_file_paths:
            df = pd.read_excel(path, header=None)
            mean_row = df[df[0] == 'mean'].iloc[0, 1:]
            mean_rows.append(mean_row.astype(float))
        mean_df = pd.DataFrame(mean_rows)
        overall_mean = mean_df.mean()
        overall_mean.to_frame(name='mean').to_excel(os.path.join(WORK_DIR, task, 'metric_result.xlsx'), index_label='metric')
